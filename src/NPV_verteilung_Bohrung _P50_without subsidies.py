...
# Neue CAPEX-Parameter
CAPEX_P10 = 8_030_000.0   # 5,21 Mio €
CAPEX_P50 = 8_440_000.0   # 5,62 Mio €
CAPEX_P90 = 8_850_000.0   # 6,03 Mio €

# Weitere Parameter unverändert
...
# Kapitalwert
npv = -capex_draw + disc_ops   # € absolut

# Quantile des NPV (in Mio €)
p10, p50, p90 = np.percentile(npv / 1e6, [10, 50, 90])

print(f"P10  : {p10:,.2f} Mio €")
print(f"P50  : {p50:,.2f} Mio €")
print(f"P90  : {p90:,.2f} Mio €")

# Histogramm
weights = np.ones_like(npv) * 100.0 / len(npv)  # jede Beobachtung = 0.01 %
plt.figure(figsize=(8, 5))
plt.hist(npv / 1e6, bins=50, weights=weights)
